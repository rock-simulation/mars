...
namespace mars {
  namespace graphics {
    ...
    void GraphicsManager::initializeOSG(void *data, bool createWindow) {
      std::cout << "[GraphicsManager::initializeOSG]" << std::endl;
      if(!initialized) {
        std::cout << "[GraphicsManager::initializeOSG] 2" << std::endl;
...
// #endif
    ...
    unsigned long GraphicsManager::new3DWindow(void *myQTWidget, bool rtt,
      ...
      std::cout << "[GraphicsManager::new3DWindow]" << std::endl;
      fprintf(stderr, "[GraphicsManager::new3DWindow]\n\n");

      visualisation = std::make_shared<EnvireVisualizerInterface>();
      ...
      mars::interfaces::ControlCenter* control = marsSim->getControlCenter();
      visualisation->displayGraph(*(control->storage->getGraph()), control->storage->getRootFrame());
    ...
    void* GraphicsManager::getQTWidget(unsigned long id) const {
      printf("dimmy implementation : %s\n", __PRETTY_FUNCTION__);
      return visualisation->getEnvireVisualizerWindow().getWidget();
      // std::vector<GraphicsWidget*>::const_iterator iter;
