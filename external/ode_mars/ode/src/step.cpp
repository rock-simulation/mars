			     ...
			     dxJoint * const *_joint, int nj, dReal stepsize)
{
  ...
  if (m > 0) {
    ...
    ALLOCA(dReal,lo,m*sizeof(dReal));
    ALLOCA(dReal,hi,m*sizeof(dReal));
    //- my hack to get motor torque
    ALLOCA(dReal,hi2,m*sizeof(dReal));
    //-
    ALLOCA(int,findex,m*sizeof(int));
    dSetZero (c,m);
    dSetValue (cfm,m,world->global_cfm);
...
#   endif
    ...
    /*
    // my hack to get torque of motor
    for (i=0; i<nj; i++) {
      if(joint[i]->feedback) {
        hi2[ofs[i]+5] = hi[ofs[i]+5];
        fprintf(stderr, "\n -- hi %d: %g %d", i, hi2[ofs[i]+5], ofs[i]+5);
      }
    }
    //-
    */
    ...
    dSolveLCP (m,A,lambda,rhs,residual,nub,lo,hi,findex);
    //for (i=0; i<m; i++) fprintf(stderr, "\n -- xxyy %d: %g", i, hi[i]);

    /*
    //- my hack to get torque of motor
    for (i=0; i<nj; i++) {
      if(joint[i]->feedback && hi2[ofs[i]+5]) {
        if(hi[ofs[i]+5] < dInfinity)  {
          fprintf(stderr, "\n -- hi %d: %g", i, hi[ofs[i]+5]);
          if((hi2[ofs[i]+5] = hi[ofs[i]+5]/hi2[ofs[i]+5])) {
            hi2[ofs[i]+5] = lambda[ofs[i]+5] / hi2[ofs[i]+5];
          }
        }
        else hi2[ofs[i]+5] = lambda[ofs[i]+5];
      }
    }
    //-
    */
...
#   endif
    ...
    for (i=0; i<nj; i++) {
      ...
      if (fb) {
        ...
        dReal data[8];
        //fprintf(stderr, "\n -- lambda %d: %g %g", i, lambda[ofs[i]+5], hi2[ofs[i]+5]);
