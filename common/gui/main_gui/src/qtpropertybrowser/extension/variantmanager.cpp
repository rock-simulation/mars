...
QStringList VariantManager::attributes(int propertyType) const
{
    if (propertyType == filePathTypeId()) {
        QStringList attr;
        attr << QLatin1String("filter") << QLatin1String("directory");
        return attr;
    }
...
{
    if (propertyType == filePathTypeId()) {
      if (attribute == QLatin1String("filter") || attribute == QLatin1String("directory"))
	return QVariant::String;
      return 0;
    }
...
QVariant VariantManager::attributeValue(const QtProperty *property, const QString &attribute)
{
    if (theValues.contains(property)) {
      ...
      if (attribute == QLatin1String("directory"))
	return theValues[property].directory;
      return QVariant();
    }
