void envmap_frag(out vec4 col, out vec4 nt) {
  ...
  acol = texture2D(envMapA, texCoord.xy*envMapScale.a);
  ...
  ant = texture2D(normalMapA, texCoord.xy*envMapScale.a);
  scale = texture2D(environmentMap, texCoord);

  col = vec4(0.7*(scale.r*rcol.rgb+scale.g*gcol.rgb+scale.b*bcol.rgb+scale.a*acol.rgb)
             +0.3*globalDiffuse.rgb, 1.0);
  nt = vec4(scale.r*rnt.rgb+scale.g*gnt.rgb+scale.b*bnt.rgb+scale.a*ant.rgb, 1.0);
}