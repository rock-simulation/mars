...
namespace mars
{
	namespace smurf
	{
		...
		SMURFLoader::SMURFLoader(lib_manager::LibManager *theManager) : interfaces::LoadSceneInterface(theManager), control(NULL)
		{
		...
		void SMURFLoader::loadEntity(configmaps::ConfigMap *it, std::string path, bool do_not_create)
		{
			...
			if (combined_path[combined_path.size() - 1] != '/')
			...
			else if (((std::string)(*it)["type"] == "smurfs") || ((std::string)(*it)["type"] == "smurfa"))
			{
				...
				if (it->hasKey("parent"))
					cfg_struct.parent = (std::string)(*it)["parent"];
				if (it->hasKey("anchor"))
					cfg_struct.anchor = (std::string)(*it)["anchor"];
															 ...
															 void *args, bool do_not_create /*=false*/)
		{
			...
			if (file_extension == ".smurfs" || file_extension == ".smurfa")
			{
				...
				for (it = map["smurfa"].begin(); it != map["smurfa"].end(); ++it)
				{ // backwards compatibility
					configmaps::ConfigMap &m = *it;
